...
# enumerate : 인덱스 + Iterable(list, dictionary, set, tuple 등) 객체 생성
...
print(list(filter(conv_pos, [1, -3, 2, 9, -7])))
print(list(filter(lambda x: abs(x) > 2, [1, -3, 2, 9, -7])))
...
# max, min : 최대값, 최소값
print(max([2, 1, 9, 7, 5, 3, 1, 6]))
print(max('My name is BHLee'))
print(min([2, 1, 9, 7, 5, 3, 1, 6]))
print(min('My name is BHLee'))

# map : 반복가능한 객체 요소를 지정한 함수 실행 후 추출
# filter와 map의 차이점 : filter은 걸러주지만, map은 함수의 결과에 해당하는 값들을 재결합해서 추출
def conv_abs(x):
    return abs(x)
